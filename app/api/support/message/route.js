...
3. UNDERSTAND IMPERFECT INPUT: Users may type with typos, broken English, slang, or incomplete sentences. Decode their meaning first. If you can reasonably infer the intent, answer directly. If you are truly unsure, ask a clarifying question before answering.
...
// ─── SMART DATA FETCHER (Handles ALL variations) ───
async function fetchBusinessData(message, businessId) {
  if (!businessId) return null;
  const lower = message.toLowerCase();

  // 1. FIRST: Check if asking about customers who owe money (debt/outstanding)
  if (/(owe|owes|owing|debt|debts|outstanding|balance|collect|unpaid)/.test(lower)) {
    // Fetch invoices with balance > 0, count unique customers
    ...
    const uniqueCustomers = new Set(data.map(inv => inv.customer_id)).size;
    return { type: 'customers_owing', count: uniqueCustomers };
  }

  // 2. Check if asking about customers (any variation)
  ...
  // 4. Check if asking about revenue
  ...
  try {
    ...
    if (businessData) {
      ...
      if (businessData.type === 'customers_owing') {
        answer = `You have ${businessData.count} customer${businessData.count !== 1 ? 's' : ''} currently owing you.`;
