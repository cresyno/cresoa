...
const customCSS = `
  ...
  .cresoa-input {
    ...
    border: 1px solid var(--cresoa-border);
    border-radius: 8px; background: var(--cresoa-bg);
    color: var(--cresoa-text); font-size: 0.85rem;
  ...
  .cresoa-prefix-dropdown {
    ...
    z-index: 9999; margin-top: 4px; overflow: hidden;
  }
  .cresoa-prefix-option {
    width: 100%; padding: 0.6rem 0.8rem;
    background: transparent; border: none;
    color: var(--cresoa-text); font-size: 0.85rem; font-weight: 600;
    cursor: pointer; text-align: left; transition: background 0.15s;
  }
  .cresoa-prefix-option:hover { background: var(--cresoa-accent-soft); color: var(--cresoa-accent); }
`
...
export default function SettingsPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (bizData.cac_number) {
          ...
          if (parts.length === 2) { cacPrefix = parts[0]; cacNumber = parts[1] } else cacNumber = bizData.cac_number
        }
        ...
        setFormData({
          ...
          tin_number: bizData.tin_number || '',
          payment_terms: bizData.payment_terms || 'Payment due within 7 days.',
        })

        if (bizData.logo_url) { setLogoPreview(bizData.logo_url); setLogoUrl(bizData.logo_url) }
      } catch (err) { console.error(err); setError('Failed to load business settings.') } finally { setLoading(false) }
    }
    load()
  }, [businessId])
  ...
  const handleRemoveLogo = () => { if (confirm('Remove your business logo?')) { setLogoFile(null); setLogoPreview(null); setLogoUrl('') } }

  const handleSave = async () => {
    setSaving(true); setError(null); setSuccess(false)
    try {
      if (formData.account_number && !/^\d{10}$/.test(formData.account_number)) { alert('Account Number must be exactly 10 digits.'); setSaving(false); return }
      if (formData.cac_number && !/^\d{5,7}$/.test(formData.cac_number)) { alert('CAC Number must be between 5 and 7 digits.'); setSaving(false); return }
      ...
      if (logoFile) {
        ...
        const { data: { publicUrl } } = supabase.storage.from('business-logos').getPublicUrl(filePath)
        finalLogoUrl = publicUrl
      } else if (!logoPreview && !logoUrl) finalLogoUrl = null
      ...
      const updates = {
        ...
        cac_number: formData.cac_number ? `${formData.cac_prefix}-${formData.cac_number}` : null,
      ...
      await supabase.from('business_activity_logs').insert({ business_id: businessId, performed_by: session.user.id, action: 'business_settings_updated', details: { name: formData.name } })

      setSuccess(true); setLogoFile(null); setBusiness({ ...business, ...updates }); setEditModal(null)
      setLastSaved(new Date().toLocaleTimeString())
      setTimeout(() => setSuccess(false), 3000)
    } catch (err) { console.error(err); setError(err.message || 'Failed to save settings') } finally { setSaving(false) }
  }

  const renderModalContent = () => {
    if (editModal === 'general') return (
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
          ...
          <label htmlFor="logo-upload-modal" style={{ padding: '0.25rem 0.8rem', borderRadius: '6px', background: 'var(--cresoa-accent)', color: '#fff', fontSize: '0.8rem', cursor: 'pointer' }}><Icon name="upload" size={14} stroke="#fff" /> Upload</label>
      ...
      <div>
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1rem', background: 'var(--cresoa-bg)', borderRadius: '8px', border: '1px solid var(--cresoa-border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            ...
            <label htmlFor="logo-upload-branding" style={{ padding: '0.4rem 1rem', borderRadius: '6px', background: 'var(--cresoa-accent)', color: '#fff', fontSize: '0.85rem', cursor: 'pointer', fontWeight: 500 }}><Icon name="upload" size={14} stroke="#fff" /> Upload</label>
        ...
        <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div><label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.2rem', color: 'var(--cresoa-text)' }}>Welcome Message</label><input type="text" name="tracking_welcome_message" value={formData.tracking_welcome_message} onChange={handleChange} className="cresoa-input" /></div>
          <div><label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.2rem', color: 'var(--cresoa-text)' }}>Footer Message</label><input type="text" name="tracking_footer_message" value={formData.tracking_footer_message} onChange={handleChange} className="cresoa-input" /></div>
        </div>
      </div>
    )
      if (editModal === 'payments') return (
      <div>
        ...
        {formData.account_number && !/^\d{10}$/.test(formData.account_number) && <p style={{ color: 'var(--cresoa-danger)', fontSize: '0.75rem', margin: '-0.5rem 0 0.8rem' }}>Account number must be exactly 10 digits.</p>}
    ...
    if (editModal === 'compliance') return (
      <div>
        <h3 style={{ marginTop: 0, color: 'var(--cresoa-text)' }}>Edit Compliance & Payment Terms</h3>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.8rem' }}>
          <div style={{ position: 'relative', width: '90px' }}>
            <button type="button" onClick={() => setShowPrefixDropdown(!showPrefixDropdown)} style={{ width: '100%', padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid var(--cresoa-border)', background: 'var(--cresoa-bg)', color: 'var(--cresoa-text)', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span>{formData.cac_prefix}</span><Icon name="chevronDown" size={14} stroke="currentColor" />
            </button>
            {showPrefixDropdown && (
              <div className="cresoa-prefix-dropdown">
                {cacPrefixes.map(prefix => (
                  <button key={prefix} type="button" onClick={() => { setFormData({ ...formData, cac_prefix: prefix }); setShowPrefixDropdown(false) }} className="cresoa-prefix-option">{prefix}</button>
                ))}
              </div>
            )}
          </div>
          <input type="text" name="cac_number" value={formData.cac_number} onChange={(e) => setFormData({ ...formData, cac_number: e.target.value.replace(/\D/g, '').slice(0, 7) })} placeholder="12345" className="cresoa-input" />
        </div>
        {formData.cac_number && !/^\d{5,7}$/.test(formData.cac_number) && <p style={{ color: 'var(--cresoa-danger)', fontSize: '0.75rem', margin: '-0.5rem 0 0.8rem' }}>CAC Number must be between 5 and 7 digits.</p>}

        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.2rem', color: 'var(--cresoa-text)' }}>TIN Number (optional)</label>
        <input type="text" name="tin_number" value={formData.tin_number} onChange={handleChange} placeholder="e.g. 12345678-0001" className="cresoa-input" style={{ marginBottom: '0.8rem' }} />

        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 500, marginBottom: '0.2rem', color: 'var(--cresoa-text)' }}>Payment Terms</label>
        <textarea name="payment_terms" value={formData.payment_terms} onChange={handleChange} rows={2} placeholder="e.g. Payment due within 7 days. Late fees apply after due date." className="cresoa-input" style={{ marginBottom: '1rem' }} />
        <p style={{ fontSize: '0.75rem', color: 'var(--cresoa-text-muted)' }}>These details will be displayed on all invoices.</p>
      </div>
    )
    return null
  }
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: '1000px', margin: '0 auto', paddingBottom: '100px', position: 'relative' }}>
      ...
      <div style={{ minHeight: '300px' }}>
        ...
        {activeTab === 'payments' && (
          <Card style={{ padding: '1.5rem' }}>
            ...
            <p style={{ marginTop: '1rem', fontSize: '0.8rem', color: 'var(--cresoa-text-muted)' }}>These details will appear on all invoices. Account number must be exactly 10 digits.</p>
          </Card>
        )}
        ...
        {activeTab === 'compliance' && (
          <Card style={{ padding: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: 'var(--cresoa-text)' }}>Compliance & Payment Terms</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', color: 'var(--cresoa-text-muted)', fontSize: '0.9rem' }}>
              <div><strong style={{ color: 'var(--cresoa-text)' }}>CAC:</strong> {formData.cac_number ? `${formData.cac_prefix}-${formData.cac_number}` : 'Not set'}</div>
              <div><strong style={{ color: 'var(--cresoa-text)' }}>TIN:</strong> {formData.tin_number || 'Not set'}</div>
              <div><strong style={{ color: 'var(--cresoa-text)' }}>Payment Terms:</strong> {formData.payment_terms || 'Not set'}</div>
            </div>
            <p style={{ fontSize: '0.8rem', color: 'var(--cresoa-text-muted)', marginTop: '1rem' }}>These details will be displayed on all invoices for trust and compliance.</p>
          </Card>
        )}
        ...
        {activeTab === 'workflow' && (
          <Card style={{ padding: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '1rem' }}>
              ...
              <p style={{ color: 'var(--cresoa-text-muted)', fontSize: '0.9rem', lineHeight: '1.6', margin: '0 0 0.5rem 0' }}>Define the 5 stages your orders go through. This will update the Production page for your industry.</p>
        ...
        <div className="cresoa-modal-overlay">
          <div className="cresoa-modal-body">
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
              <button onClick={() => setEditModal(null)} style={{ padding: '0.6rem 1.2rem', borderRadius: '8px', border: '1px solid var(--cresoa-border)', background: 'transparent', color: 'var(--cresoa-text)', cursor: 'pointer', fontWeight: 500, fontSize: '0.9rem' }}>Cancel</button>
              <button onClick={handleSave} disabled={saving} style={{ padding: '0.6rem 1.5rem', borderRadius: '8px', border: 'none', background: 'var(--cresoa-primary)', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem', opacity: saving ? '0.7' : '1' }}>{saving ? 'Saving...' : 'Save Changes'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Bottom save bar */}
 <div style={{
        position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--cresoa-card)',
        borderTop: '1px solid var(--cresoa-border)', padding: '0.75rem 1.5rem',
        display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 100,
        boxShadow: '0 -4px 12px rgba(0,0,0,0.04)', flexWrap: 'wrap', gap: '0.5rem',
      }}>
        <div>
        ...
        <button onClick={handleSave} disabled={saving} style={{ padding: '0.6rem 1.8rem', borderRadius: '8px', border: 'none', background: 'var(--cresoa-primary)', color: '#fff', fontWeight: 600, cursor: 'pointer', fontSize: '0.9rem', opacity: saving ? '0.7' : '1' }}>{saving ? 'Saving...' : 'Save Changes'}</button>
      </div>
