...
export default function GroupsPage() {
  ...
  const loadGroups = async () => {
    ...
    try {
      ...
      // Fetch business details (sector, plan, name)
      ...
      // Check if groups are allowed for this business sector and plan
      ...
      // Fetch groups with member counts and total balance
      ...
      // For each group, get member orders and compute stats
      const groupsWithStats = await Promise.all(
        (groupsData || []).map(async (g) => {
          ...
          if (ordersError) {
            return { ...g, memberCount: 0, totalBalance: 0, deliveredCount: 0 }
          }

          const memberCount = orders.length
          const totalBalance = orders.reduce(
            (sum, o) => sum + Math.max(0, (o.price || 0) - (o.amount_paid || 0)),
            0
          )
      ...
      <div style={{ padding: '1.5rem', maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
        <div style={{ padding: '3rem 2rem', background: 'var(--color-card)', borderRadius: '16px', border: '1px solid var(--color-border)' }}>
          ...
          {!isFashion ? (
            ...
            <a
              href={`/dashboard/subscription?business_id=${getCurrentBusinessId() || ''}`}
    ...
    return (
      <div style={{ padding: '1.5rem', maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
        <div style={{ background: 'var(--color-card)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--color-border)' }}>
          ...
          <button onClick={loadGroups} style={{ marginTop: '1rem', padding: '0.5rem 1.5rem', background: 'var(--color-accent)', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Retry</button>
        </div>
      </div>
    )
  }
  ...
  const totalBalance = groups.reduce((sum, g) => sum + g.totalBalance, 0)
  const totalDelivered = groups.reduce((sum, g) => sum + g.deliveredCount, 0)
  const totalOrders = groups.reduce((sum, g) => sum + g.memberCount, 0)
    ...
    <div style={{ padding: '1.5rem', maxWidth: '1200px', margin: '0 auto', color: 'var(--color-text)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', margin: '0.1rem 0 0', fontSize: '0.85rem' }}>
            {totalGroups} group{totalGroups !== 1 ? 's' : ''} · {totalMembers} members · ₦{totalBalance.toLocaleString()} total owed
          </p>
        </div>
        <a
          href={`/dashboard/groups/new?business_id=${getCurrentBusinessId() || ''}`}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '0.5rem', marginBottom: '1.5rem' }}>
        <div style={{ background: 'var(--color-card)', padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Groups</div>
          <div style={{ fontWeight: '700', fontSize: '1.1rem' }}>{totalGroups}</div>
        </div>
        <div style={{ background: 'var(--color-card)', padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Members</div>
          <div style={{ fontWeight: '700', fontSize: '1.1rem' }}>{totalMembers}</div>
        </div>
        <div style={{ background: 'var(--color-card)', padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Owed</div>
          <div style={{ fontWeight: '700', fontSize: '1.1rem', color: totalBalance > 0 ? 'var(--color-danger)' : 'var(--color-success)' }}>
            ₦{totalBalance.toLocaleString()}
          </div>
        </div>
        <div style={{ background: 'var(--color-card)', padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
          <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Progress</div>
      ...
      {groups.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem 2rem', background: 'var(--color-card)', borderRadius: '12px', border: '1px dashed var(--color-border)' }}>
          <span style={{ fontSize: '3rem', display: 'block', marginBottom: '0.5rem' }}>👥</span>
          <h3 style={{ fontSize: '1rem', fontWeight: '600', margin: '0 0 0.3rem' }}>No groups yet</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
          {groups.map((g) => {
            const progress = g.memberCount > 0 ? Math.round((g.deliveredCount / g.memberCount) * 100) : 0
            const isOverdue = g.due_date && new Date(g.due_date) < new Date()
            return (
              <div
                key={g.id}
                style={{
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ fontWeight: '600', fontSize: '1rem', color: 'var(--color-text)' }}>{g.group_name}</div>
                  <span style={{ fontSize: '0.65rem', fontWeight: '600', padding: '0.1rem 0.6rem', borderRadius: '12px', background: g.status === 'completed' ? 'var(--color-success)' : 'var(--color-accent)', color: '#fff' }}>
                    {g.status === 'completed' ? 'Completed' : 'Active'}
                  </span>
                </div>

                <div style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', marginTop: '0.2rem' }}>
                  {g.coordinator?.name ? `Coordinator: ${g.coordinator.name}` : 'No coordinator'}
                </div>

                {g.due_date && (
                  <div style={{ fontSize: '0.75rem', color: isOverdue ? 'var(--color-danger)' : 'var(--color-text-muted)', marginTop: '0.2rem' }}>
                    Due: {new Date(g.due_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                    {isOverdue && ' ⚠️ Overdue'}
                  </div>
                )}

                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.3rem', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)' }}>{g.memberCount} members</span>
                  <span style={{ fontSize: '0.7rem', fontWeight: '500', color: g.totalBalance > 0 ? 'var(--color-danger)' : 'var(--color-success)' }}>
                    ₦{g.totalBalance.toLocaleString()} {g.totalBalance > 0 ? 'owed' : '✓ paid'}
                  </span>
                </div>

                <div style={{ marginTop: '0.3rem', height: '4px', background: 'var(--color-bg)', borderRadius: '4px' }}>
                  <div style={{ width: `${progress}%`, height: '100%', background: 'var(--color-success)', borderRadius: '4px' }} />
                </div>

                <div style={{ marginTop: '0.8rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap', borderTop: '1px solid var(--color-border)', paddingTop: '0.5rem' }}>
                  <a
                    href={`/dashboard/groups/${g.id}?business_id=${getCurrentBusinessId() || ''}`}
                    style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', textDecoration: 'none', border: '1px solid var(--color-border)', padding: '0.1rem 0.6rem', borderRadius: '4px' }}
                  >
                    View
                  </a>
                  <a
                    href={`/dashboard/groups/${g.id}/edit?business_id=${getCurrentBusinessId() || ''}`}
                    style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', textDecoration: 'none', border: '1px solid var(--color-border)', padding: '0.1rem 0.6rem', borderRadius: '4px' }}
                  >
                    Edit
                  </a>
                  <button
                    ...
                    style={{ fontSize: '0.7rem', color: 'var(--color-danger)', background: 'none', border: 'none', cursor: 'pointer', padding: '0.1rem 0.6rem' }}
                  >
                    Delete
                  </button>
                </div>
