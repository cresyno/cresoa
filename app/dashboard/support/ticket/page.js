...
export default function ClientTicketPage() {
  ...
  // 1. Pre-fill the email and fetch tickets on load
  useEffect(() => {
    const init = async () => {
  ...
  // 2. Submit with Email instead of Business ID
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/support/ticket', {
        ...
        // We send the email instead of the business_id
    ...
    <div style={{ padding: '1.5rem', maxWidth: '800px', margin: '0 auto', background: 'var(--color-bg)', minHeight: 'calc(100vh - 80px)' }}>
      ...
      <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: '16px', padding: '1.5rem', marginBottom: '2rem', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {/* NEW EMAIL FIELD - Pre-filled, editable */}
          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--color-text-muted)', marginBottom: '0.25rem' }}>Your Email Address</label>
