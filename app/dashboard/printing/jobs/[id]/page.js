...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ padding:'0.3rem 0.8rem', borderRadius:'20px', background:`${colors[status] || '#666'}20`, color:colors[status] || '#666', fontWeight:700, fontSize:'0.75rem' }}>
...
export default function PrintJobDetailPage() {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        if (assignedIds.length > 0) {
          const { data: staffData } = await supabase.from('staff').select('*').in('id', assignedIds)
          setStaffList(staffData || [])
        } else setStaffList([])

        const { data: filesData } = await supabase.from('job_files').select('*').eq('job_id', jobId).order('created_at', { ascending: false })
        setFiles(filesData || [])

        const { data: revisionsData } = await supabase.from('job_revisions').select('*').eq('job_id', jobId).order('created_at', { ascending: false })
        setRevisions(revisionsData || [])

        const { data: paymentsData } = await supabase.from('payment_records').select('*').eq('job_id', jobId).order('created_at', { ascending: false })
        setPayments(paymentsData || [])

        const { data: invoicesData } = await supabase.from('invoices').select('*').eq('job_id', jobId)
  ...
  const isOverdue = job?.deadline && new Date(job.deadline) < new Date() && job.status !== 'delivered'
  ...
  // ─── File upload (real Supabase Storage) ───
  const handleFileUpload = async (e) => {
    const file = e.target.files?.[0]
    if (!file) return
    try {
      const path = `print-jobs/${job.id}/${Date.now()}_${file.name}`
      ...
      const { error: dbError } = await supabase.from('job_files').insert({ job_id: job.id, filename: file.name, file_url: fileUrl, uploaded_by: (await supabase.auth.getUser()).data.user.id })
      if (dbError) throw dbError
      const { data: freshFiles } = await supabase.from('job_files').select('*').eq('job_id', job.id).order('created_at', { ascending: false })
      setFiles(freshFiles || [])
  ...
  if (error || !job) return <div style={{ minHeight:'70vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'var(--cresoa-bg)', textAlign:'center' }}><h2>Couldn't load job</h2><p style={{ color:'var(--cresoa-text-muted)' }}>{error || 'Job not found'}</p><button onClick={() => router.push(`/dashboard/printing/jobs?business_id=${businessIdFromUrl}`)} className="cresoa-primary-button">Back to Jobs</button></div>

  const customerName = customer?.name || `${customer?.first_name || ''} ${customer?.last_name || ''}`.trim() || 'Customer'

  return (
    <div style={{ padding:'1rem', maxWidth:'900px', margin:'0 auto', background:'var(--cresoa-bg)', minHeight:'100vh', paddingBottom:'140px' }}>
      {/* Back */}
      <button onClick={() => router.push(`/dashboard/printing/jobs?business_id=${businessIdFromUrl}`)} style={{ display:'inline-flex', alignItems:'center', gap:'0.3rem', background:'none', border:'none', cursor:'pointer', color:'var(--cresoa-text-muted)', marginBottom:'1rem' }}><Svg name="back" size={16} stroke="currentColor" /> Back to Jobs</button>
      ...
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.5rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginTop:'0.8rem' }}>
          ...
          {isOverdue && <span style={{ padding:'0.2rem 0.6rem', borderRadius:'12px', background:'var(--cresoa-danger-soft)', color:'var(--cresoa-danger)', fontWeight:600, fontSize:'0.75rem' }}>Overdue</span>}
        </div>
      </div>

      {/* Primary Actions (top, non-sticky) */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginBottom:'1rem' }}>
        <button onClick={() => router.push(`/dashboard/printing/invoices/new?job_id=${job.id}&business_id=${businessIdFromUrl}`)} className="cresoa-primary-button" style={{ background:'var(--cresoa-accent)' }}><Svg name="file" size={16} stroke="#fff" /> Generate Invoice</button>
        <button onClick={() => shareWhatsApp('status')} className="cresoa-primary-button" style={{ background:'#25D366' }}><Svg name="whatsapp" size={16} stroke="#fff" /> WhatsApp Update</button>
      </div>

      {/* Job Information (moved up) */}
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.2rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        <DetailRow label="Deadline" value={job.deadline ? formatDate(job.deadline) : 'Not set'} highlight={!job.deadline || isOverdue} />
      ...
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.2rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        {invoices.length > 0 && <div style={{ marginTop:'0.8rem' }}>{invoices.map(inv => <button key={inv.id} onClick={() => router.push(`/dashboard/printing/invoices/${inv.id}?business_id=${businessIdFromUrl}`)} style={{ display:'block', width:'100%', padding:'0.4rem', background:'var(--cresoa-bg)', border:'1px solid var(--cresoa-border)', borderRadius:'6px', marginBottom:'0.3rem', textAlign:'left', cursor:'pointer', color:'var(--cresoa-text)' }}>{inv.invoice_number} · {formatMoney(inv.total)}</button>)}</div>}
      </div>
    ...
    {/* Production */}
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.2rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        {staffList.length > 0 && <div style={{ marginTop:'0.8rem' }}><p style={{ fontSize:'0.75rem', fontWeight:600, margin:'0 0 0.3rem' }}>Assigned Staff:</p><div style={{ display:'flex', flexWrap:'wrap', gap:'0.3rem' }}>{staffList.map(staff => <span key={staff.id} style={{ padding:'0.2rem 0.6rem', borderRadius:'12px', background:'var(--cresoa-accent-soft)', color:'var(--cresoa-accent)', fontSize:'0.75rem', fontWeight:600 }}>{staff.name}</span>)}</div></div>}
        <div style={{ marginTop:'0.8rem' }}>
          <p style={{ fontSize:'0.75rem', fontWeight:600, margin:'0 0 0.3rem' }}>Stage History:</p>
          {revisions.length > 0 ? revisions.map(rev => <div key={rev.id} style={{ padding:'0.3rem', borderBottom:'1px solid var(--cresoa-border)', fontSize:'0.8rem' }}>{rev.revision_number}. {rev.notes || 'Stage change'} · {formatDate(rev.created_at)}</div>) : <p style={{ fontSize:'0.8rem', color:'var(--cresoa-text-muted)' }}>No stage changes yet.</p>}
        </div>
      </div>
      ...
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.2rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        {trackingLink ? (
          ...
          <div style={{ textAlign:'center', padding:'1rem' }}>
            <p style={{ color:'var(--cresoa-text-muted)', fontSize:'0.85rem', margin:'0 0 0.8rem' }}>Generate a link so your customer can track their job.</p>
            <button onClick={generateTrackingLink} className="cresoa-primary-button" style={{ background:'var(--cresoa-primary)' }}><Svg name="link" size={16} stroke="#fff" /> Generate Tracking Link</button>
          </div>
        )}
      </div>
      ...
      <div style={{ background:'var(--cresoa-surface)', borderRadius:'16px', padding:'1.2rem', marginBottom:'1rem', border:'1px solid var(--cresoa-border)', boxShadow:'var(--shadow-sm)' }}>
        ...
        <label style={{ display:'inline-block', padding:'0.6rem 1rem', background:'var(--cresoa-accent)', color:'#fff', borderRadius:'8px', cursor:'pointer', fontWeight:600, fontSize:'0.9rem', marginBottom:'0.8rem' }}><Svg name="upload" size={14} stroke="#fff" /> Upload Artwork<input type="file" onChange={handleFileUpload} style={{ display:'none' }} /></label>
        {files.length > 0 ? <div style={{ display:'flex', flexDirection:'column', gap:'0.4rem' }}>{files.map(file => <div key={file.id} style={{ padding:'0.4rem', background:'var(--cresoa-bg)', borderRadius:'6px', display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid var(--cresoa-border)' }}><span style={{ fontSize:'0.8rem' }}>{file.filename} {file.is_approved && '✓ Approved'}</span><a href={file.file_url} target="_blank" rel="noopener" style={{ color:'var(--cresoa-accent)', fontSize:'0.8rem' }}>View</a></div>)}</div> : <p style={{ color:'var(--cresoa-text-muted)', fontSize:'0.85rem', margin:0 }}>No artwork yet. Upload your first design to keep everything organized.</p>}
      </div>
