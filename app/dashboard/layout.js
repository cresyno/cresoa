...
function DashboardLayoutContent({ children }) {
  ...
  // ─── Load business data (SAFE VERSION - No auto-redirect loops) ───
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ⚠️ SAFETY NET: Only redirect to onboarding if the user truly has NO business.
        // If they have a business, NEVER redirect to onboarding.
        if (!businessData) {
          ...
          // If they haven't created a business yet, take them to onboarding. Otherwise, let them enter.
          if (!hasOnboarded) {
        ...
        // If businessData was found but userRole is empty, fetch it
        ...
        // ⚠️ CRITICAL FIX: Do NOT redirect to onboarding on error. Just stop loading.
