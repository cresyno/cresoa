...
export default function CustomerDetailPage({ params }) {
  ...
  const refreshCustomer = async () => {
    try {
      ...
      const result = await response.json()
      if (!response.ok) {
        throw new Error(result.error || 'Unable to refresh customer')
      }
    ...
    >
      ...
      <div className="customer-header">
        ...
        <div className="customer-header-actions">
          ...
          >
            <Icon name={editing ? 'x' : 'edit-2'} size={15} />
            {editing ? 'Close' : 'Edit'}
          </button>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          <p className="number navy">₦{(stats?.totalSpent || 0).toLocaleString()}</p>
        ...
        <div className="stat-card">
          <p className="number navy">₦{(stats?.avg || 0).toLocaleString()}</p>
      ...
      {editing && (
        <form onSubmit={handleSave} className="edit-form">
          ...
          <div className="form-group">
            <label>Notes</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Customer notes..." />
          </div>

          {isFashion && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', margin: '1rem 0 0.6rem' }}>
                <Icon name="ruler" size={16} stroke="var(--color-accent)" />
                <h3 style={{ color: 'var(--color-text)', fontSize: '0.9rem', margin: 0 }}>Measurements</h3>
              </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.7rem', flexWrap: 'wrap' }}>
            <button type="submit" className="customer-btn customer-btn-primary" disabled={saving}>
              <Icon name={saving ? 'refresh-cw' : 'check'} size={15} />
              {saving ? 'Saving changes...' : 'Save customer'}
            </button>
            <button type="button" className="customer-btn" onClick={() => setEditing(false)}>
              <Icon name="x" size={15} /> Cancel
            </button>
          </div>

          {message && (
            <p style={{ color: message.startsWith('Customer updated') ? 'var(--color-success)' : 'var(--color-danger)', marginTop: '0.6rem', fontSize: '0.8rem' }}>
              {message}
            </p>
          )}
        </form>
      ...
      <div className="orders-section">
        <div className="title">
          ...
          <a href={`/dashboard/orders/new?business_id=${businessId}&customer_id=${customer.id}`}>
        ...
        {orders.length === 0 ? (
          <div style={{ padding: '1.5rem', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
            <Icon name="package" size={24} stroke="var(--color-text-muted)" />
          ...
          orders.map((order) => {
            ...
            return (
              <div key={order.id} className="order-row">
                <div>
                  <div className="order-title">{order.title || 'Untitled order'}</div>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  {balance > 0 && (
                    <div style={{ fontSize: '0.65rem', color: 'var(--color-danger)', marginTop: '0.15rem' }}>
      ...
      {!editing && customer.notes && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.45rem' }}>
            <Icon name="file-text" size={15} stroke="var(--color-accent)" />
            <strong style={{ fontSize: '0.8rem' }}>Notes</strong>
          </div>
          <p style={{ margin: 0, fontSize: '0.85rem', lineHeight: 1.5, color: 'var(--color-text-muted)', whiteSpace: 'pre-wrap' }}>
            {customer.notes}
          </p>
        </div>
      )}
      ...
      >
        <button
          ...
          disabled={deleting}
        >
          <Icon name={deleting ? 'refresh-cw' : 'trash-2'} size={15} />
      ...
      <style jsx>{`
        ...
        @media (max-width: 700px) {
          ...
          .customer-header-actions .customer-btn,
