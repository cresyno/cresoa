  ...
  return (
    ...
    >
      <style jsx>{`
        ...
        h1, h2, h3, h4 { font-family: 'Fraunces', serif; font-weight: 700; letter-spacing: -0.02em; }
        ...
        .btn {
          ...
          background: var(--color-accent); color: #0F2B4A; box-shadow: 0 4px 16px rgba(212,165,42,0.3); font-family: inherit;
        }
        .btn:hover { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(212,165,42,0.4); }
        .btn-outline { background: transparent; color: var(--color-text); border: 1px solid var(--color-border); box-shadow: none; }
        .btn-outline:hover { background: var(--color-card); border-color: var(--color-accent); }
        .btn-secondary { background: var(--color-primary); color: #fff; box-shadow: 0 4px 16px rgba(15,43,74,0.3); }
        ...
        .navbar { background: var(--color-card); border-bottom: 1px solid var(--color-border); padding: 0.8rem 1.5rem; display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; z-index: 1000; backdrop-filter: blur(12px); flex-wrap: wrap; gap: 0.5rem; }
        ...
        .hamburger { display: none; background: none; border: none; color: var(--color-text); cursor: pointer; padding: 0.2rem 0.5rem; }
        ...
        .hero .subhead { color: #C8D4E3; font-size: 1.1rem; line-height: 1.7; margin: 0 0 2rem; max-width: 540px; }
        .hero .cta-group { display: flex; flex-wrap: wrap; gap: 0.8rem; }
        .hero .trust-badges { display: flex; flex-wrap: wrap; gap: 0.5rem 1.5rem; margin-top: 1.5rem; color: #A0B4C9; font-size: 0.8rem; }
        .hero .trust-badges span { display: flex; align-items: center; gap: 0.3rem; }
        .trust-bar { display: flex; justify-content: center; gap: 2rem; flex-wrap: wrap; padding: 1.2rem 0; border-top: 1px solid rgba(255,255,255,0.08); margin-top: 2.5rem; }
        ...
        .feature-card { background: var(--color-card); border-radius: 16px; padding: 1.8rem 1.2rem; border: 1px solid var(--color-border); text-align: center; transition: all 0.3s ease; box-shadow: var(--shadow-sm); }
        ...
        .step-item .number { background: var(--color-primary); color: #fff; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 1rem; flex-shrink: 0; }
        ...
        .pricing-card.popular .popular-badge { background: var(--color-accent); color: #0F2B4A; font-size: 0.6rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; padding: 0.15rem 0.8rem; border-radius: 20px; display: inline-block; margin-bottom: 0.8rem; }
        ...
        .faq-answer { max-height: 0; overflow: hidden; transition: max-height 0.35s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.3s ease; opacity: 0; padding: 0 1.2rem; color: var(--color-text-muted); font-size: 0.9rem; line-height: 1.7; }
      ...
      <nav className="navbar">
        ...
        <div className="nav-actions">
          ...
          <button className="hamburger" onClick={toggleMobileMenu}>{ICONS.menu}</button>
        </div>
      </nav>
      ...
      <div className={`mobile-drawer ${mobileMenuOpen ? 'open' : ''}`}>
        ...
        <Link href="/signup" className="nav-btn nav-btn-primary">Start Beta</Link>
        <div style={{ paddingTop: '0.5rem' }}><ThemeToggle /></div>
      </div>
      ...
      <section id="why" className="container" style={{ padding: '3.5rem 1.5rem' }}>
        <div className="section-title"><h2>Why Nigerian businesses choose Cresoa</h2><p>Real tools for real businesses. Built for your workflow.</p></div>
        <div className="why-grid" ref={statsRef}>
          <div className="why-item"><div className="number">{ICONS.users} {counts.users}</div><div className="label">Beta users</div></div>
          <div className="why-item"><div className="number">{ICONS.layers} {counts.industries}</div><div className="label">Industries live</div></div>
          <div className="why-item"><div className="number">{ICONS.globe} 🇳🇬</div><div className="label">Built in Nigeria</div></div>
          <div className="why-item"><div className="number">{ICONS.clock} {counts.days}</div><div className="label">Days of free access</div></div>
          <div className="why-item"><div className="number">{ICONS.smartphone} 📱</div><div className="label">Mobile-first</div></div>
        </div>
      </section>

      {/* ─── FEATURES ─── */}
      <section id="features" className="container" style={{ padding: '2rem 1.5rem 3.5rem' }}>
        <div className="section-title"><h2>Everything you need to run smoothly</h2><p>One platform for your customers, orders, payments, and production.</p></div>
        <div className="card-grid">
          {[
            ...
            <div key={f.title} className="feature-card">
              <div className="icon-wrapper">{ICONS[f.icon] || ICONS.fileText}</div>
              <h3>{f.title}</h3>
              <p>{f.desc}</p>
            </div>
          ))}
      ...
      <section className="container" style={{ padding: '2rem 1.5rem' }}>
        <div className="section-title"><h2>Get started in 3 simple steps</h2><p>From signup to running your business in minutes.</p></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
          <div className="step-item"><span className="number">1</span><div><h4>Create your account</h4><p>Sign up free and choose your business type—Fashion, Repairs, or Manufacturing.</p><span className="time">{ICONS.clock} 2 minutes</span></div></div>
          <div className="step-item"><span className="number">2</span><div><h4>Add customers & orders</h4><p>Import your customers, add their details, and create your first order or repair job.</p><span className="time">{ICONS.clock} 5 minutes</span></div></div>
      ...
      <section id="pricing" className="container" style={{ padding: '3.5rem 1.5rem' }}>
        <div className="section-title"><h2>Simple, transparent pricing</h2><p>Start free, pay only when you are ready to grow.</p></div>
        <div className="pricing-grid">
          <div className="pricing-card"><p className="plan-name">Free</p><div className="price">₦0</div><p className="description">Perfect for getting started</p><ul><li><span className="check">{ICONS.check}</span> 20 customers</li><li><span className="check">{ICONS.check}</span> 50 orders</li><li><span className="check">{ICONS.check}</span> Basic order tracking</li><li><span className="cross">{ICONS.x}</span> Staff accounts</li><li><span className="cross">{ICONS.x}</span> WhatsApp integration</li></ul><Link href="/signup" className="btn btn-outline" style={{ width: '100%', justifyContent: 'center', textDecoration: 'none' }}>Start Free</Link></div>
          <div className="pricing-card popular"><div className="popular-badge">Most Popular</div><p className="plan-name">Starter</p><div className="price">₦3,500 <span>/month</span></div><p className="description">For growing businesses</p><ul><li><span className="check">{ICONS.check}</span> Unlimited customers</li><li><span className="check">{ICONS.check}</span> Unlimited orders</li><li><span className="check">{ICONS.check}</span> 2 staff accounts</li><li><span className="check">{ICONS.check}</span> WhatsApp integration</li><li><span className="check">{ICONS.check}</span> Customer tracking links</li></ul><Link href="/signup" className="btn" style={{ width: '100%', justifyContent: 'center', textDecoration: 'none' }}>Start Beta</Link></div>
          <div className="pricing-card"><p className="plan-name">Pro</p><div className="price">₦9,500 <span>/month</span></div><p className="description">For established businesses</p><ul><li><span className="check">{ICONS.check}</span> Everything in Starter</li><li><span className="check">{ICONS.check}</span> 10 staff accounts</li><li><span className="check">{ICONS.check}</span> Advanced analytics</li><li><span className="check">{ICONS.check}</span> Data export (Excel/PDF)</li><li><span className="check">{ICONS.check}</span> Priority support</li></ul><Link href="/signup" className="btn btn-secondary" style={{ width: '100%', justifyContent: 'center', textDecoration: 'none' }}>Start Beta</Link></div>
        </div>
      </section>

      {/* ─── FAQ ─── */}
      <section id="faq" className="container" style={{ padding: '2rem 1.5rem 3.5rem', maxWidth: '720px' }}>
        <div className="section-title"><h2>Frequently asked questions</h2><p>Everything you need to know about Cresoa.</p></div>
        <div>
          {faqs.map((item, i) => (
            <div key={i} className="faq-item">
              <div className="faq-header" onClick={() => toggleFaq(i)}>{item.q}<span className={`arrow ${openFaqIndex === i ? 'open' : ''}`}>+</span></div>
              <div className={`faq-answer ${openFaqIndex === i ? 'open' : ''}`}>{item.a}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="final-cta">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8rem', justifyContent: 'center' }}>
          <Link href="/signup" className="btn" style={{ fontSize: '1.05rem', padding: '0.9rem 2.5rem', textDecoration: 'none' }}>Start Beta {ICONS.arrowRight}</Link>
          <Link href="/login" className="btn btn-outline" style={{ fontSize: '1.05rem', padding: '0.9rem 2rem', background: 'transparent', color: '#fff', borderColor: 'rgba(255,255,255,0.3)', textDecoration: 'none' }}>Log in</Link>
        </div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', flexWrap: 'wrap', marginTop: '1.2rem', color: '#A0B4C9', fontSize: '0.75rem' }}>
          <span>{ICONS.check} 90-day beta</span><span>{ICONS.check} No credit card</span><span>{ICONS.check} Cancel anytime</span>
        </div>
      </section>
      ...
      <footer className="footer">
        <div className="container">
          ...
          <p style={{ opacity: 0.6, fontSize: '0.7rem', marginTop: '0.5rem' }}>© {new Date().getFullYear()} Cresoa · All rights reserved.</p>
        </div>
      </footer>
    </main>
  )
