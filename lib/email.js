 ...
 */
export async function sendBetaInviteEmail(to, inviterName, businessName, acceptLink, expiresInDays = 7) {
  const subject = `🎉 You're invited to beta test ${businessName} on Cresoa`

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2>🎉 You're invited to beta test!</h2>
      <p><strong>${inviterName}</strong> has invited you to join the <strong>${businessName}</strong> beta program on Cresoa.</p>
      <p>As a beta tester, you'll get full access to all Pro features <strong>free for 90 days</strong>.</p>
      <a href="${acceptLink}" style="display: inline-block; padding: 12px 24px; background: #1E3A5F; color: #fff; text-decoration: none; border-radius: 6px;">
        Accept Beta Invitation
      </a>
      <p style="margin-top: 20px; color: #666; font-size: 14px;">
        This link expires in ${expiresInDays} days.<br>
        If you don't have a Cresoa account yet, you'll be prompted to create one.
      </p>
      <hr style="border: none; border-top: 1px solid #eee;" />
      <p style="font-size: 12px; color: #999;">
        If you didn't expect this invitation, you can ignore this email.
