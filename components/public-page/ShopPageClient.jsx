        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', maxWidth: '400px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
                ...
                {cartItems.map((item) => (
                  <div key={item.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <button onClick={() => removeFromCart(item.name)} style={{ background: 'none', border: 'none', color: 'red', cursor: 'pointer', fontWeight: 700 }}>×</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem' }}>
              <button onClick={() => setCartOpen(false)} style={{ flex: 1, padding: '0.7rem', borderRadius: '8px', border: '1px solid #E5E7EB', background: '#fff', cursor: 'pointer' }}>Close</button>
              <button
                onClick={() => {
                  setCartOpen(false)
                ...
                style={{ flex: 1, padding: '0.7rem', borderRadius: '8px', border: 'none', background: '#25D366', color: '#fff', fontWeight: 700, cursor: 'pointer' }}
              >
